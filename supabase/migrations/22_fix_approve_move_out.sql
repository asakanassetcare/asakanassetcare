-- ============================================================================
-- Migration 22: Fix approve_move_out flow
--
-- Before:
--   - approve_move_out() from migration 09 duplicated side effects already owned
--     by trg_move_out_approved from migration 03.
--   - Both paths could create settlements for the same move_out_id, causing a
--     duplicate key rollback.
--   - Contract close logic diverged between RPC and trigger.
--
-- After:
--   - The trigger owns side effects: contract close, room release, settlement,
--     and notification.
--   - The RPC owns only permission checks and the status transition.
--   - Zero-balance move-outs are marked settled immediately without creating a
--     pending zero-amount settlement.
-- ============================================================================

-- 1. Trigger: single source of truth for move-out approval side effects.
...
-- 2. RPC: permission check + status transition only.
